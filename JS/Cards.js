...
function predictCards() {
    ...
    let pack = cardStuff.packs.horde.rookieOnTheBattlefield
    var prog = typeof parseInt(document.getElementById('cardRng').value) === "number" ? document.getElementById('cardRng').value : 0;
    for (let i = 0; i < 10; i++) {
        let rngGen = new Math.seedrandom(document.getElementById('playerID').value + "cardPack_" + "rookieOnTheBattlefield" + "_" + (parseInt(prog) + i));
        let cacheWeight = [];
        let cacheContent = [];
